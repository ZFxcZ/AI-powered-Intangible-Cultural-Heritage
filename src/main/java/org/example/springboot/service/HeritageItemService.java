...
//        }
        ...
        if (queryDTO.getCurrentPage() == null || queryDTO.getCurrentPage() <= 0) {
        ...
        try {
            ...
            log.info("=== 开始执行分页查询 ===");
            log.info("分页参数：currentPage={}, size={}", queryDTO.getCurrentPage(), queryDTO.getSize());
            log.info("查询条件：title={}, category={}, region={}, status={}, orderBy={}, orderDirection={}",
                    queryDTO.getTitle(), queryDTO.getCategory(), queryDTO.getRegion(),
                    queryDTO.getStatus(), queryDTO.getOrderBy(), queryDTO.getOrderDirection());

            Page<HeritageItem> itemPage = heritageItemMapper.selectPageWithConditions(page, queryDTO);

            log.info("=== 分页查询完成，总数：{} ===", itemPage.getTotal());
            ...
            String rootCauseMsg = getRootCauseMessage(e);
            log.error("=== MyBatis 系统异常 ===");
            log.error("根本原因：{}", rootCauseMsg);
            log.error("queryDTO={}", queryDTO, e);
            throw new ServiceException("数据库查询失败：" + rootCauseMsg);
        } catch (Exception e) {
            log.error("=== 查询作品列表失败 ===");
            log.error("queryDTO={}", queryDTO, e);
            throw new ServiceException("查询失败，请稍后重试");
