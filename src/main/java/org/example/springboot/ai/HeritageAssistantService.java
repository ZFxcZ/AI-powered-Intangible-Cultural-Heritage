...
//    }
    ...
    /**
     * AI生成流式对话（带会话记忆）：Deepseek-v3
     * @param sessionId
     * @param userMessage
     * @param userId
     * @return
     */
    public Flux<String> chatStream(String sessionId, String userMessage, Long userId) {
        log.info("开始AI对话，sessionId: {}, userMessage: {}, userId: {}", sessionId, userMessage, userId);

        sessionService.saveMessage(sessionId, "user", userMessage);

        String systemPrompt = PromptManage.HERITAGE_ASSISTANT_PROMPT +
                "\n\n## 当前用户信息：\n- 用户ID: " + (userId != null ? userId : "未登录");

        // AI构建流式响应：Deepseek-v3
        Flux<String> responseFlux = chatClient.prompt()
                .system(systemPrompt)
                .user(userMessage)
                .advisors(advisorSpec -> advisorSpec
    ...
    public String chat(String sessionId, String userMessage, Long userId) {
        log.info("开始AI对话（非流式），sessionId: {}, userMessage: {}, userId: {}", sessionId, userMessage, userId);
