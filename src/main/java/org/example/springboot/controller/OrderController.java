...
public class OrderController {
    ...
    public Result<String> getPayUrl(
            @Parameter(description = "订单 ID") @PathVariable Long id,
            @Parameter(description = "支付方式") @RequestParam String payType) {
        Long userId = JwtTokenUtils.getCurrentUserId();
        log.info("获取支付链接，订单 ID: {}, 用户 ID: {}, 支付方式：{}", id, userId, payType);

        // 构建完整的支付 URL（包含 token）
        String baseUrl = "/shop/order/" + id + "/pay";
        String fullUrl = baseUrl + "?payType=" + payType;
...
//            @Parameter(description = "支付方式") @RequestParam String payType, HttpServletResponse httpServletResponse) throws IOException {
//        Long userId = JwtTokenUtils.getCurrentUserId();
//        log.info("支付订单，订单ID: {}, 用户ID: {}, 支付方式: {}", id, userId, payType);
////        orderService.payOrder(id, userId, payType);
//        String form = orderService.alipay(id, userId, payType);
//        httpServletResponse.setContentType("text/html;charset=" + AlipayProperties.charset);
...
//        orderService.payOrder(id, userId, payType);
    ...
    public void returnCallback(HttpServletRequest request, HttpServletResponse response) {
        orderService.returnCallback(request, response);
    }

    @Autowired
    private AlipayProperties alipayProperties;

    @PostMapping("/notify") // 更精确地使用 @PostMapping
